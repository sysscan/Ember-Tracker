document.addEventListener('DOMContentLoaded', function() {
    if (typeof L === 'undefined') {
        document.getElementById('map').innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100%;color:#888;">Failed to load map library. Please refresh.</div>';
        return;
    }
    ...
    function getConfidenceColor(confidence) {
        ...
        if (c === 'h' || c === 'high' || parseInt(confidence) >= 80) return '#ff3535';
        if (c === 'n' || c === 'nominal' || parseInt(confidence) >= 50) return '#ff6b35';
        return '#ffb835';
    }
    ...
    function getConfidenceClass(confidence) {
        ...
        if (c === 'n' || c === 'nominal' || parseInt(confidence) >= 50) return 'nominal';
        return 'low';
    }
        ...
        return `
            <div class="popup-content">
                ...
                <div class="popup-row">
                    ...
                    <span class="popup-value">${fire.latitude.toFixed(5)}°</span>
                </div>
                <div class="popup-row">
                    <span class="popup-label">Longitude</span>
                    <span class="popup-value">${fire.longitude.toFixed(5)}°</span>
                </div>
                <div class="popup-row">
                    <span class="popup-label">Brightness</span>
                    <span class="popup-value">${fire.bright_ti4 || fire.brightness || '--'} K</span>
                </div>
                <div class="popup-row">
                ...
                <div class="popup-row">
                    ...
                    <span class="popup-value">${getConfidenceLabel(fire.confidence)}</span>
                </div>
                <div class="popup-row">
                    <span class="popup-label">Date</span>
                    <span class="popup-value">${fire.acq_date || '--'}</span>
                </div>
                <div class="popup-row">
                    <span class="popup-label">Time (UTC)</span>
                    <span class="popup-value">${formatTime(fire.acq_time)}</span>
                </div>
                <div class="popup-row">
                    <span class="popup-label">Satellite</span>
                    <span class="popup-value">${fire.satellite || '--'}</span>
                </div>
            </div>
        `;
    }
    ...
    function renderFireMarkers(data) {
        ...
        document.getElementById('fireCount').textContent = data.length.toLocaleString();
        document.getElementById('lastUpdate').textContent = new Date().toLocaleTimeString('en-US', {
            hour: '2-digit',
            minute: '2-digit'
        });

        renderFireList(data.slice(0, 50));
    }
    ...
    function renderFireList(data) {
        ...
        container.innerHTML = data.map((fire, i) => `
            <div class="fire-item" data-index="${i}">
                <div class="fire-item-header">
                    <span class="fire-item-coords">${fire.latitude.toFixed(3)}°, ${fire.longitude.toFixed(3)}°</span>
                    <span class="fire-item-confidence ${getConfidenceClass(fire.confidence)}">${getConfidenceLabel(fire.confidence)}</span>
                </div>
                <div class="fire-item-meta">${fire.acq_date} ${formatTime(fire.acq_time)} UTC • FRP: ${fire.frp || '--'} MW</div>
            </div>
        `).join('');
    ...
    const regionPresets = {
        ...
        california: { w: -125, s: 32, e: -114, n: 42, center: [37, -120], zoom: 6 },
        australia: { w: 112, s: -44, e: 154, n: -10, center: [-25, 135], zoom: 4 },
